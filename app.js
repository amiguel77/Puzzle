...
function isFist(landmarks) {
  ...
  for (const [tipIdx, mcpIdx] of pairs) {
    if (
      dist2D(landmarks[tipIdx], wrist) <
      dist2D(landmarks[mcpIdx], wrist)
    ) {
      curled++;
    }
  }
...
function computeHandFrame(indexTipA, indexTipB) {
  ...
  const width =
    Math.min(canvas.width, maxX) - x;

  const height =
    Math.min(canvas.height, maxY) - y;
...
function drawCountdownOverlay(box) {
  const elapsed =
    (performance.now() - countdown.startedAt) / 1000;
  ...
  ctx.font =
    `${Math.max(
      48,
      Math.min(box.width, box.height) * 0.4
...
function applyPhotoboothEffect(imageData) {
  ...
  for (let i = 0; i < d.length; i += 4) {
    ...
    let v =
      gray * PHOTOBOOTH_CONTRAST_ALPHA +
...
function finishCountdownAndCapture(box) {
  ...
  const mirroredCtx =
    mirroredFrame.getContext("2d");

  mirroredCtx.save();

  mirroredCtx.translate(
    mirroredFrame.width,
    0
  );
  ...
  const tileW =
    Math.floor(cropCanvas.width / GRID);

  const tileH =
    Math.floor(cropCanvas.height / GRID);
...
function isNearOwnCell(
  ...
  const correctX =
    box.x + piece.col * tileW;

  const correctY =
    box.y + piece.row * tileH;
...
function displaceCellOccupant(
  ...
  const cellX =
    box.x + targetCol * tileW;

  const cellY =
    box.y + targetRow * tileH;

  const occupant =
    puzzle.pieces.find((p) => {
      if (
        p === piece ||
        p.displacing
      ) {
  ...
  const targetX =
    box.x +
    targetSlot.col * tileW +
    jitterX;

  const targetY =
    box.y +
    targetSlot.row * tileH +
    jitterY;
...
function updateProgressBadge() {
  if (appState !== "puzzle") {
    progressBadge.classList.remove(
      "visible",
      "solved"
    );
  ...
  progressBadge.classList.add(
    "visible"
  );

  progressBadge.classList.toggle(
    "solved",
    puzzle.solved
  );
}
...
function resetLoaderUI() {
  ...
  loaderText.textContent =
    "loading HandLandmarker model…";

  loaderRetry.classList.add(
    "hidden"
  );
...
async function boot() {
  ...
  const watchdogMs =
    LOAD_TIMEOUT_MS * 2 + 5000;

  const watchdog =
    setTimeout(() => {
      if (!settled) {
        showLoaderError(
          "The loading process is taking too long. Click retry or check your connection."
        );
      }
    }, watchdogMs);
